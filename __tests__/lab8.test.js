describe("Basic user flow for Website", () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let text = itext["_remoteObject"].value;
    expect(text).toMatch("Remove from Cart");
    await button.click();
  }, 2500);
  ...
  it("Checking number of items in cart on screen", async () => {
    ...
    for (let i = 0; i < prod.length; i++) {
      shadow = await prod[i].getProperty("shadowRoot");
      button = await shadow.$("button");
      await button.click();
    }

    let count = await page.$("#cart-count");
    let countInner = await count.getProperty("innerText");
    let cartCount = countInner["_remoteObject"].value;
  ...
  it("Checking number of items in cart on screen after reload", async () => {
    ...
    let element = await page.$("#cart-count");
    let countInner = await element.getProperty("innerText");
    let cartCount = countInner["_remoteObject"].value;
    expect(cartCount).toMatch("20");
  }, 10000);
  ...
  it("Checking number of items in cart on screen after removing from cart", async () => {
    ...
    let element = await page.$("#cart-count");
    let inner = await element.getProperty("innerText");
    let cartCount = inner["_remoteObject"].value;
  ...
  it("Checking number of items in cart on screen after reload", async () => {
    ...
    for (let i = 0; i < prod.length; i++) {
      ...
      innerText = await button.getProperty("innerText");
      text = innerText["_remoteObject"].value;
      expect(text).toMatch("Add to Cart");
    }
    let element = await page.$("#cart-count");
    let countInner = await element.getProperty("innerText");
    let cartCount = countInner["_remoteObject"].value;
